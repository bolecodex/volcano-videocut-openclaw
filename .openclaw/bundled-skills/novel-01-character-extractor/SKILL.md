...
> **API 调用方式**：本 skill 中的图像生成（角色立绘），通过 shell 调用 CLI 包装器执行：
> ```bash
> python /Users/m007/codes/long_video_skills/skills-openclaw/mcp-proxy/xskill_api.py generate --model "fal-ai/bytedance/seedream/v4.5/text-to-image" --prompt "角色提示词" --image_size portrait_16_9
...
使用角色提示词（`prompt` 字段）调用图像生成模型，获取角色立绘图片 URL。

**生成后必须立即下载到本地**（远程 URL 会过期）：

```bash
# 下载到 assets/characters/{角色id}.png
mkdir -p "{项目目录}/assets/characters/"
curl -L -o "{项目目录}/assets/characters/{角色id去掉@}.png" "https://生成的图片URL"
```

**路径规则**：
- 角色 ID 去掉 `@` 前缀作为文件名，如 `@szj_suwan` → `szj_suwan.png`
- 最终路径示例: `assets/characters/szj_suwan.png`

写入 YAML 时同时填写 `image_url` 和 `image_path`：

```yaml
image_url: "https://生成的图片URL"
image_path: "assets/characters/szj_suwan.png"
image_status: completed
```

> **重要**：`image_path` 是相对于项目目录的路径，前端优先使用 `image_path`。`image_url` 保留作为来源记录和兜底。
...
```bash
python /Users/m007/codes/long_video_skills/skills-openclaw/novel-01-character-extractor/scripts/generate_gallery.py \
...
```bash
# 基本用法（自动推导输出路径）
python /Users/m007/codes/long_video_skills/skills-openclaw/novel-01-character-extractor/scripts/generate_gallery.py --input "{项目目录}/{剧本名}_角色资产.yaml"
