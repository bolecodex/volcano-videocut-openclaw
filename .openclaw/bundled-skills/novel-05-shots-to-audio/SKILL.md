...
生成音频后，**必须先下载到本地**（远程 URL 会过期），然后更新 YAML：

```bash
# 下载到 assets/audio/{镜头ID}_line_{序号}.mp3
mkdir -p "{项目目录}/assets/audio/"
curl -L -o "{项目目录}/assets/audio/{镜头ID}_line_0.mp3" "https://cdn.minimax.chat/audio/xxx.mp3"
curl -L -o "{项目目录}/assets/audio/{镜头ID}_line_1.mp3" "https://cdn.minimax.chat/audio/yyy.mp3"
```

**路径规则**：
- `{镜头ID}` = 镜头 ID，如 `SC_01_001`
- `{序号}` = 台词在 lines 中的索引（从 0 开始）
- 最终路径示例: `assets/audio/SC_01_001_line_0.mp3`

将下载后的本地路径和远程 URL **同时**写入 YAML：
...
lines:
  - speaker: 旁白
    ...
    audio_path: "assets/audio/SC_01_001_line_0.mp3"
    audio_status: completed
  - speaker: 苏晚
    text: 夫君，今晚，替我多砍几刀。
    emotion: 颤抖、决绝
    audio_url: "https://cdn.minimax.chat/audio/yyy.mp3"
    audio_path: "assets/audio/SC_01_001_line_1.mp3"
    audio_status: completed
```

> **重要**：`audio_path` 是相对于项目目录的路径，前端优先使用 `audio_path`。`audio_url` 保留作为来源记录和兜底。
