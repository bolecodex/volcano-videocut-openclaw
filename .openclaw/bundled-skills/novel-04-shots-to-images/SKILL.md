...
生成完成后，**必须先下载图片到本地**，然后更新 YAML 文件。远程 URL 会过期，本地文件不会。

### 4.1 下载图片到本地

每个镜头生成成功后，立即下载图片到本地 workspace：

```bash
# 下载到 shots/{场景名}/shot_{编号}.png
mkdir -p "{项目目录}/shots/{场景名}/"
curl -L -o "{项目目录}/shots/{场景名}/shot_{编号}.png" "https://生成的图片URL"
```

**路径规则**：
- `{场景名}` = 场景 YAML 文件名（不含 `.yaml` 后缀），如 `SC_01_开篇悬念`
- `{编号}` = 镜头编号，如 `001`、`002`
- 最终路径示例: `shots/SC_01_开篇悬念/shot_001.png`

### 4.2 更新分镜 YAML 文件

下载完成后，更新 YAML。**`image_path` 是相对于 `shots/` 目录的路径**：
...
> **重要**：`image_url` 保留作为来源记录，`image_path` 是实际使用的本地路径。前端优先使用 `image_path`。
