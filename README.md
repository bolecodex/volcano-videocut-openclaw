...
5. **开始剪辑** — 一键生成投流素材
6. **Agent 对话** — 在右侧聊天面板与 AI 交互，使用 @ 引用技能、模版；**@ 文件夹** 可选中 `video/` 下含多集的原片目录（整夹分析），无需逐个 @ 视频文件。多集投流请让 Agent **对目录只跑一次** `analyze_video.py`（勿加 `--single`），再 **一次** `ffmpeg_cut`，这样成片**只在开头有一个全局钩子**。；长任务可看「当前处理」与详细日志，必要时点「停止」后修改提示词再发
