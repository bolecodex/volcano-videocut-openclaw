...
```bash
cd {baseDir}/remotion-project
npm install
```

> `{baseDir}` 是本 skill 的文件夹路径，即 `.cursor/skills/novel-07-remotion/`。
...
检查 `{baseDir}/remotion-project/node_modules` 是否存在。如不存在：

```bash
cd {baseDir}/remotion-project && npm install
```
...
```bash
cd {baseDir}/remotion-project && npx tsx scripts/prepare-data.ts \
  --project "{项目目录}" \
  --scene "SC_01" \
...
```bash
cd {baseDir}/remotion-project && npx tsx scripts/render.ts \
  --project "{项目目录}" \
  --output "{项目目录}/output/remotion" \
  --scene "SC_01"
```
...
Remotion 项目位于 `{baseDir}/remotion-project/`，可自行修改 React 组件来扩展效果：
...
```bash
cd {baseDir}/remotion-project && npx remotion studio
```
